...
EXCLUDED_FILE_NAMES = {".env", "secrets.yaml"}
EXCLUDED_SUFFIXES = {".cookie", ".cookies", ".har", ".session", ".token"}
EXCLUDED_SENSITIVE_STEMS = {"cookie", "cookies", "session", "sessions", "token", "tokens"}
...
def is_excluded(relative_path: Path) -> bool:
    ...
    if relative_path.stem.casefold() in EXCLUDED_SENSITIVE_STEMS:
        return True
    return name.lower().endswith(tuple(EXCLUDED_SUFFIXES))
