...
async def test_real_recorder_history_import_with_uppercase_entry_id(recorder_mock, hass):
    ...
    manager = HistorySyncManager(
        hass,
        SimpleNamespace(entry_id="01M10K4M0P890X641ZCED5ZD07", options={}),
        RuntimeData(website_provider=SimpleNamespace(async_fetch=fetch)),
    )

    status = await manager.async_sync(force=True)
    await async_recorder_block_till_done(hass)
    statistic_id = "tanita_healthplanet:01m10k4m0p890x641zced5zd07_1"
    ...
    assert [row["mean"] for row in result[statistic_id]] == [60, 61, 62, 63, 64]
    assert [row["min"] for row in result[statistic_id]] == [60, 61, 62, 63, 64]
    assert [row["max"] for row in result[statistic_id]] == [60, 61, 62, 63, 64]
